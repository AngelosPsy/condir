...
csReport <- function(csCompareObj = NULL, csSensitivityObj = NULL, save = FALSE,
    ...
    if (!base::is.null(csCompareObj)){
      ...
      paired <- base::ifelse(base::as.character(
        csCompareObj$freq.results[["method"]]) == "Paired t-test", TRUE, FALSE)
      p.val <- csCompareObj$freq.results[["p.value"]]

      if (paired && p.val < 0.05){
        inter <- "Those results suggest that there are statistically significant differences between cs1 and cs2."
      } else if (paired && p.val >= 0.05){
        inter <- "Those results suggest that there are no statistically significant differences between cs1 and cs2."
      } else if (!paired && p.val < 0.05){
        inter <- "Those results suggest that there are statistically significant between group differences."
      } else if (!paired && p.val >= 0.05){
        inter <- "Those results suggest that there are no statistically significant between group differences."
      }
